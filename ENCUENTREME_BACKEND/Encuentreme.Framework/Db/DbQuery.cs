...
namespace Encuentreme.Framework.Db;
...
public sealed class DbQuery
{
    ...
    {
        // En PostgreSQL, para funciones que retornan tablas, es más seguro usar SELECT * FROM funcion(@params)
        ...
        {
            // DateTime → DateOnly so Npgsql sends PostgreSQL 'date' not 'timestamp'
            object? mapped = value switch
            {
                DateTime dt => DateOnly.FromDateTime(dt),
                _ => value
            };
            cmd.Parameters.AddWithValue(name, mapped ?? DBNull.Value);
        }
        return cmd;
    }
        ...
        while (reader.Read())
        {
            ...
            for (int i = 0; i < reader.FieldCount; i++)
            {
                ...
                if (prop != null && !reader.IsDBNull(i))
                {
                    ...
                    if (prop.PropertyType == typeof(int) && val is not int) val = Convert.ToInt32(val);
    ...
    public ReturnValue ToResult()
    {
        ...
        for (int i = 0; i < reader.FieldCount; i++)
        {
            ...
            else if (name == "p_id_publicacion") res.Id = Convert.ToInt32(val);
        }
        return res;
    }
